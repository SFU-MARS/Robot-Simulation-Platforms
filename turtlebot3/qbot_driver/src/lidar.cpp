...
t_uint32 scan_mode =
    1;  // the mode to be used. 0 - RANGING_MEASUREMENT_MODE_NORMAL, 1 -
        // RANGING_MEASUREMENT_MODE_INTERPOLATED
t_uint32
    num_scan_points;  // number of points to be sent out of the scan message
...
rcl_interfaces::msg::SetParametersResult set_parameters_callback(
  ...
  // Loop through the parameters....can happen if set_parameters_atomically() is
  // called
  for (const auto& parameter : parameters) {
    if (parameter.get_name().compare("uri") == 0) {
      ...
      if (node_running) {
        ...
        result.reason = "Cannot change this parameter while node is running.";
                 ...
                 (parameter.as_int() >= NUMBER_OF_RANGING_MEASUREMENT_MODES)) {
        std::ostringstream error_stream;

        error_stream << "Paramater value out of range. Valid values must be "
                        "betwwen 0 and "
                     << (NUMBER_OF_RANGING_MEASUREMENT_MODES - 1);

        result.successful = false;
        result.reason = error_stream.str();
      } else if ((parameter.as_int() == RANGING_MEASUREMENT_MODE_NORMAL) &&
                 (num_scan_points != max_num_measurements)) {
        std::ostringstream error_stream;

        error_stream << "num_scan_points must be " << max_num_measurements
                     << " when scan_mode = 0.";

        result.successful = false;
        result.reason = error_stream.str();
      } else {
        // Normally we don't set a variable in the callback (we call
        // parameter.get_param() to get the parameter at the point we need to
        // use it). But we have to set the "scan_mode" global variable at this
        // point, so that it can be used by "num_scan_points" parameter change.
        scan_mode = parameter.as_int();
      }
    } else if (parameter.get_name().compare("num_scan_points") == 0) {
      if ((scan_mode == RANGING_MEASUREMENT_MODE_NORMAL) &&
          (parameter.as_int() != max_num_measurements)) {
        std::ostringstream error_stream;
        ...
        error_stream << "num_scan_points must be between 0 and "
        ...
        // Normally we don't set a variable in the callback (we call
        // parameter.get_param() to get the parameter at the point we need to
        // use it). But we have to set the "num_scan_points" global variable at
        // this point, so that it can be used by "scan_mode" parameter change.
        num_scan_points = parameter.as_int();
      }
    } else if (parameter.get_name().compare("max_interpolated_distance") == 0) {
      if ((parameter.as_double() < min_distance) ||
          (parameter.as_double() > max_distance)) {
        std::ostringstream error_stream;

        error_stream << "max_interpolated_distance must be between "
    ...
    } else if (parameter.get_name().compare("num_allowable_invalid_data") ==
               0) {
      if ((parameter.as_int() < 0) ||
          (parameter.as_int() > max_num_measurements)) {
        std::ostringstream error_stream;

        error_stream << "num_allowable_invalid_data must be between 0 and "
                     << max_num_measurements;
...
int main(int argc, char** argv) {
  ...
  double publish_rate =
      12;  // Hz. Leishen M10P can only do 12 Hz at device level. So set to 10
           // Hz for the node seems reasonable.
  ...
  try {
    parameter_cb =
        node->add_on_set_parameters_callback(set_parameters_callback);
  } catch (const std::bad_alloc& e) {
    RCLCPP_ERROR(node->get_logger(), "Error setting up parameters callback. %s",
                 e.what());
  }
  auto param_desc = rcl_interfaces::msg::ParameterDescriptor{};

  param_desc.description =
      "The URI to interface with the Lidar. On QBot Platform, the default value of \
\"serial://localhost:0?device='/dev/lidar',baud='512000',word='8',parity='none',stop='1',flow='none'\" \
should be used.";
  node->declare_parameter(
      "uri",
      "serial://localhost:0?device='/dev/"
      "lidar',baud='512000',word='8',parity='none',stop='1',flow='none'",
      param_desc);
  uri = node->get_parameter("uri").as_string();
  // RCLCPP_INFO(node->get_logger(), "Parameter uri = %s", uri.c_str());

  param_desc.description =
      "The number of scan points to be sent out in the /scan message.";
  param_desc.additional_constraints =
      "This parameter must be 1680 if scan_mode is NORMAL (0). This parameter "
      "can be between 0 and 1680, if scan_mode is INTERPOLATED (1).";
  node->declare_parameter("num_scan_points", (int)max_num_measurements,
                          param_desc);

  param_desc.description =
      "The scan mode to use. Recommended to use INTERPOLATION (1) mode for "
      "QBot Platform.";
  param_desc.additional_constraints =
      "This parameter must be NORMAL (0) or INTERPOLATED (1).";
  ...
  param_desc.description =
      "When INTERPOLATION mode is used (scan_mode := 1), \
...
angle to the desired heading will be used.";
  param_desc.additional_constraints =
      "This parameter must be between 0 and 10.0.";
...
for which interpolation will be used. Beyond this difference, the distance and quality will be set to zero.";
  ...
  param_desc.description =
      "When NORMAL mode is used (scan_mode := 0), \
  ...
  if (result >= 0) {
    ...
    while (rclcpp::ok()) {
      ...
      */
      ...
      max_interpolated_distance =
          node->get_parameter("max_interpolated_distance").as_double();
      max_interpolated_angle =
          node->get_parameter("max_interpolated_angle").as_double();
      num_allowable_invalid_data =
          node->get_parameter("num_allowable_invalid_data").as_int();

      result = leishen_m10p_read(
          lidar_sensor, static_cast<t_ranging_measurement_mode>(scan_mode),
          max_interpolated_distance, max_interpolated_angle, measurements,
          num_scan_points);

      /*
      afterReadTime = node->get_clock()->now();
      RCLCPP_INFO(node->get_logger(), "lidar_read took %lf seconds",
      afterReadTime.seconds() - beforeReadTime.seconds());
      */

      if (result >= 0 && ((t_uint32)result >=
                          (num_scan_points -
                           ((1 - scan_mode) * num_allowable_invalid_data)))) {
        msgs.header.frame_id = "laser_link";
        ...
        msgs.ranges.clear();
        // Fill msgs.ranges in reverse order to be compatible with the CCW
        // convention
        ...
        // RCLCPP_INFO(node->get_logger(), "Publishing scan with %ld number of
        // data", msgs.ranges.size());
        ...
        if (result < 0) {
          if (result != -QERR_WOULD_BLOCK) {
            msg_get_error_messageA(NULL, result, error_message,
                                   ARRAY_LENGTH(error_message));
            RCLCPP_ERROR(node->get_logger(),
                         "Error reading lidar data: %d -> %s", result,
                         error_message);
          }
        } else {
          // RCLCPP_INFO(node->get_logger(), "Too many invalid data from scan.
          // Expected %d, got %d. Not sending scan message",
          // max_num_measurements, result);
        }
      }
    ...
    msg_get_error_messageA(NULL, result, error_message,
                           ARRAY_LENGTH(error_message));
    RCLCPP_ERROR(node->get_logger(), "Error opening the Lidar: %d -> %s",
