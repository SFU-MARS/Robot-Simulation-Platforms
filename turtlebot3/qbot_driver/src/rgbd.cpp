...
int main(int argc, char **argv) {
  ...
  param_desc.description =
      "The camera number of the RGBD camera. On QBot Platform, it should "
      "normally be 0";
  ...
  param_desc.description =
      "The requested frame width of the RGB stream. Only certain combinations "
      "of frame_width_rgb, frame_height_rgb, and frame_rate parameters can be "
      "used. Refer to the following Additional constraints for details.";
  ...
  // RCLCPP_INFO(node->get_logger(), "Parameter frame_width_rgb = %d",
  // frame_width_rgb);

  param_desc.description =
      "The requested frame height of the RGB stream. Only certain combinations "
      "of frame_width_rgb, frame_height_rgb, and frame_rate parameters can be "
      "used. Refer to the following Additional constraints for details.";
  param_desc.additional_constraints =
      "Possible values:\n\t320 x 180 @???\n\t320 x 240 @???\n\t424 x 240 "
  ...
  // RCLCPP_INFO(node->get_logger(), "Parameter frame_height_rgb = %d",
  // frame_height_rgb);

  param_desc.description =
      "The requested frame width of the Depth stream. Only certain "
      "combinations of frame_width_depth, frame_height_depth, and frame_rate "
      "parameters can be used. Refer to the following Additional constraints "
      "for details.";
  param_desc.additional_constraints =
      "Possible values:\n\t256 x 144 @???\n\t848 x 100 @???\n\t424 x 240 "
      "@???\n\t480 x 270 @???\n\t640 x 360 @???\n\t640 x 480 @???\n\t848 x 480 "
  ...
  // RCLCPP_INFO(node->get_logger(), "Parameter frame_width_depth = %d",
  // frame_width_depth);

  param_desc.description =
      "The requested frame height of the Depth stream. Only certain "
      "combinations of frame_width_depth, frame_height_depth, and frame_rate "
      "parameters can be used. Refer to the following Additional constraints "
      "for details.";
  param_desc.additional_constraints =
  ...
  // RCLCPP_INFO(node->get_logger(), "Parameter frame_height_depth = %d",
  // frame_height_depth);

  param_desc.description =
      "The requested frame rate of the CSI camera. Only certain combinations "
      "of frame_width_rgb, frame_height_rgb, and frame_rate parameters can be "
      "used. Refer to the following Additional constraints for details.";
  param_desc.additional_constraints =
      "Possible values:\n\t640 x 400 @210.0\n\t640 x 480 @180.0\n\t1280 x 720 "
  ...
  buffer_rgb = (t_uint8 *)memory_allocate(frame_width_rgb * frame_height_rgb *
                                          3 * sizeof(t_uint8));
  buffer_depth = (t_uint16 *)memory_allocate(
      frame_width_depth * frame_height_depth * sizeof(t_uint16));
  ...
  if ((buffer_rgb != NULL) && (buffer_depth != NULL)) {
    ...
    if (result >= 0) {
      ...
      result_rgb = video3d_stream_open(
          capture, VIDEO3D_STREAM_COLOR, 0, frame_rate, frame_width_rgb,
          frame_height_rgb, IMAGE_FORMAT_ROW_MAJOR_INTERLEAVED_BGR,
          IMAGE_DATA_TYPE_UINT8, &rgb_stream);
      result_depth = video3d_stream_open(
          capture, VIDEO3D_STREAM_DEPTH, 0, frame_rate, frame_width_depth,
          frame_height_depth, IMAGE_FORMAT_ROW_MAJOR_GRAYSCALE,
          IMAGE_DATA_TYPE_UINT16, &depth_stream);
      if ((result_rgb >= 0) && (result_depth >= 0)) {
        ...
        if (result >= 0) {
          ...
          while (rclcpp::ok()) {
            ...
            if (result >= 0) {
              ...
              if (result >= 0) {
                color_matrix = cv::Mat(frame_height_rgb, frame_width_rgb,
                                       CV_8UC3, buffer_rgb);
                // Check if grabbed frame is actually full with some content
                if (!color_matrix.empty()) {
                  msg =
                      cv_bridge::CvImage(
                          hdr, sensor_msgs::image_encodings::BGR8, color_matrix)
                          .toImageMsg();
                ...
                RCLCPP_ERROR(node->get_logger(),
                             "Error getting data from rgb frame: %d -> %s",
              ...
              if (result != -QERR_WOULD_BLOCK) {
                msg_get_error_messageA(NULL, result, error_message,
                                       ARRAY_LENGTH(error_message));
                RCLCPP_ERROR(node->get_logger(),
                             "Error getting frame from rgb stream: %d -> %s",
                             result, error_message);
              }

              // Try to send the same frame as last time
              msg = cv_bridge::CvImage(hdr, sensor_msgs::image_encodings::BGR8,
                                       color_matrix)
                        .toImageMsg();
              color_pub.publish(msg);
            }
            ...
            if (result >= 0) {
              ...
              if (result >= 0) {
                depth_matrix = cv::Mat(frame_height_depth, frame_width_depth,
                                       CV_16UC1, buffer_depth);

                // Check if grabbed frame is actually full with some content
                if (!depth_matrix.empty()) {
                  msg = cv_bridge::CvImage(hdr,
                ...
                msg_get_error_messageA(NULL, result, error_message,
                                       ARRAY_LENGTH(error_message));
                RCLCPP_ERROR(node->get_logger(),
                             "Error getting data from depth frame: %d -> %s",
                             result, error_message);
              }
              ...
              if (result != -QERR_WOULD_BLOCK) {
                msg_get_error_messageA(NULL, result, error_message,
                                       ARRAY_LENGTH(error_message));
                RCLCPP_ERROR(node->get_logger(),
                             "Error getting frame from depth stream: %d -> %s",
                             result, error_message);
              }

              // Try to send the same frame as last time
              msg = cv_bridge::CvImage(
                        hdr, sensor_msgs::image_encodings::MONO16, depth_matrix)
                        .toImageMsg();
              depth_pub.publish(msg);
            }
            ...
            try {
              ...
              // ImageTransport and the Node seems to both handle the Ctrl-C
              // interrupt. One of them tries to shtudown and causes a
              // rclcpp::execptions::RCLError exception with the error being:
              //      failed to create guard condition: the given context is not
              //      valid, either rcl_init() was not called or rcl_shutdown()
              //      was called.
              //
              // At this point, we're shutting down anyways, so just ignore the
              // exception.
          ...
          msg_get_error_messageA(NULL, result, error_message,
                                 ARRAY_LENGTH(error_message));
          RCLCPP_ERROR(node->get_logger(), "Error starting stream: %d -> %s",
                       result, error_message);
        }
        ...
        if (result_rgb < 0) {
          msg_get_error_messageA(NULL, result_rgb, error_message,
                                 ARRAY_LENGTH(error_message));
          RCLCPP_ERROR(node->get_logger(),
                       "Error opening RGB stream. result: %d -> %s", result_rgb,
                       error_message);
        }

        if (result_depth < 0) {
          msg_get_error_messageA(NULL, result_depth, error_message,
                                 ARRAY_LENGTH(error_message));
          RCLCPP_ERROR(node->get_logger(),
                       "Error opening Depth stream. result: %d -> %s",
                       result_depth, error_message);
        }
      }
      ...
      msg_get_error_messageA(NULL, result, error_message,
                             ARRAY_LENGTH(error_message));
      RCLCPP_ERROR(node->get_logger(), "Error opening device. result: %d -> %s",
                   result, error_message);
    }
