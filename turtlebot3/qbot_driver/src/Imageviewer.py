...
class ImageSubscriber(Node):
    def __init__(self, topicName, imageType):
        ...
        if imageType == "CompressedImage":
            ...
            self.subscription = self.create_subscription(
                Image, str(topicName), self.Image_callback, self.qcar_qos_profile
            )
        else:
...
def main(args=None):
    ...
    topic_name = input(
        "Please input the name of the image node you will like to subscribe to (example: /qcar/csi_left): "
    )
    print(
        "Please input image message type.\nFor compressed image topic use: CompressedImage , for regular Image topic use: Image"
    )
