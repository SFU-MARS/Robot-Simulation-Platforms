...
int main(int argc, char **argv) {
  ...
  image_transport::Publisher image_pub =
      it.advertise("camera/downward_image", 1);
  ...
  param_desc.description =
      "The camera number of the CSI camera. On QBot Platform, it should "
      "normally be 0";
  ...
  param_desc.description =
      "The requested frame width of the CSI camera. Only certain combinations "
      "of frame_width, frame_height, and frame_rate parameters can be used. "
      "Refer to the following Additional constraints for details.";
  ...
  param_desc.description =
      "The requested frame height of the CSI camera. Only certain combinations "
      "of frame_width, frame_height, and frame_rate parameters can be used. "
      "Refer to the following Additional constraints for details.";
  param_desc.additional_constraints =
      "Possible values:\n\t640 x 400 @210.0\n\t640 x 480 @180.0\n\t1280 x 720 "
  ...
  // RCLCPP_INFO(node->get_logger(), "Parameter frame_height = %d",
  // frame_height);
  ...
  image_buffer =
      (t_uint8 *)memory_allocate(frame_width * frame_height * sizeof(t_uint8));
  ...
  result = video_capture_open(uri, frame_rate, frame_width, frame_height,
                              IMAGE_FORMAT_ROW_MAJOR_GRAYSCALE,
                              IMAGE_DATA_TYPE_UINT8, &capture, NULL, 0);
  ...
  if (result >= 0) {
    ...
    if (result >= 0) {
      ...
      while (rclcpp::ok()) {
        ...
        if (result >= 0) {
          image_matrix =
              cv::Mat(frame_height, frame_width, CV_8UC1, image_buffer);

          // Check if grabbed frame is actually full with some content
          if (!image_matrix.empty()) {
            msg = cv_bridge::CvImage(hdr, sensor_msgs::image_encodings::MONO8,
                                     image_matrix)
                      .toImageMsg();
          ...
          if (result != -QERR_WOULD_BLOCK) {
            ...
            RCLCPP_ERROR(node->get_logger(), "Error reading image: %d -> %s",
        ...
        try {
          ...
          //      valid, either rcl_init() was not called or rcl_shutdown() was
          //      called.
          //
      ...
      msg_get_error_messageA(NULL, result, error_message,
                             ARRAY_LENGTH(error_message));
      RCLCPP_ERROR(node->get_logger(), "Error starting capture: %d -> %s",
                   result, error_message);
    }
