...
class QBotPlatformDriverInterface : public rclcpp::Node {
 public:
  QBotPlatformDriverInterface() : Node("qbot_platform_driver_interface") {
    int result;

    other_cb_group_ = this->create_callback_group(
    ...
    try {
      parameter_cb = this->add_on_set_parameters_callback(std::bind(
          &QBotPlatformDriverInterface::set_parameters_callback, this, _1));
    } catch (const std::bad_alloc &e) {
      RCLCPP_ERROR(this->get_logger(),
                   "Error setting up parameters callback. %s", e.what());
    }
    auto param_desc = rcl_interfaces::msg::ParameterDescriptor{};

    param_desc.description =
        "The URI to interface with the QBot Platform Driver. The default value of \
\"tcpip://localhost:18888\" should be used.";
    this->declare_parameter("driver_uri", "tcpip://localhost:18888",
                            param_desc);
    driver_uri = this->get_parameter("driver_uri").as_string();
    // RCLCPP_INFO(this->get_logger(), "Parameter driver_uri = %s",
    // driver_uri.c_str());

    param_desc.description =
        "Set this to \"true\" to enable, \"false\" to disable the motors.";
    param_desc.additional_constraints =
        "This arm_mode parameter must be NORMAL (0) to use this parameter";
    this->declare_parameter("arm_robot", false, param_desc);
    ...
    param_desc.additional_constraints =
        ...
        "Limit";
    this->declare_parameter("speed_limit_mode", 1, param_desc);
    speed_limit_mode = this->get_parameter("speed_limit_mode").as_int();
    // RCLCPP_INFO(this->get_logger(), "Parameter speed_limit_mode = %d",
    // speed_limit_mode);

    param_desc.description =
        "The body speed duration (in seconds) of the QBot Platform.";
    this->declare_parameter("body_speed_duration", 0.5, param_desc);
    body_speed_duration =
        this->get_parameter("body_speed_duration").as_double();
    // RCLCPP_INFO(this->get_logger(), "Parameter body_speed_duration = %lf",
    // body_speed_duration);

    param_desc.description =
        "Set this to \"true\" to hold QBot Platform in current position, "
        "\"false\" to allow QBot Platform to move.";
    // param_desc.additional_constraints = "This arm_mode parameter must be
    // NORMAL (0) to use this parameter";
    ...
    param_desc.description = "Whether to publish baselink->odom transform.";
    this->declare_parameter("pub_tf", false, param_desc);
    pub_tf = this->get_parameter("pub_tf").as_bool();

    param_desc.description =
        "Amount of time to predict ahead for the baselink->odom transform";
    this->declare_parameter("odom_pred_dt", 0.0, param_desc);
    odom_pred_dt = this->get_parameter("odom_pred_dt").as_double();
    ...
    /* Number of units to be sent/received with each pstream_send or
     * pstream_receive call */
    ...
    */
    ...
    /* The FIFO queue buffer sizes. These FIFO queues will be used if
     * PSTREAM_FLAG_MULTITHREADED is set. */
    ...
    /*
    Use blocking I/O. The pstream API is always non-blocking, but setting this
    flag will create a separate thread in which blocking I/O is done. But that
    thread communicates with the pstream_send and pstream_receive via the
    send/receive FIFO queues. The pstream_send and pstream_receive are always
    non-blocking and setting this flag will not change their mode of operation.
    */
    options.flags |= PSTREAM_FLAG_MULTITHREADED;

    /* Set the other flags accordingly. Since we are looking at latency, use the
     * most recent flags */
    ...
    if (result == 0) {
      ...
      speed_feedback_publisher_ =
          this->create_publisher<geometry_msgs::msg::TwistStamped>(
              "qbot_speed_feedback", 1, pub_options);
      battery_state_publisher_ =
          this->create_publisher<sensor_msgs::msg::BatteryState>(
              "qbot_battery", 1, pub_options);
      // Cartographer subsribes to the /imu topic
      imu_publisher_ =
          this->create_publisher<sensor_msgs::msg::Imu>("imu", 1, pub_options);
      joint_state_publisher_ =
          this->create_publisher<sensor_msgs::msg::JointState>("qbot_joint", 1,
                                                               pub_options);
      // XL: create an odom publish
      odom_publisher_ = this->create_publisher<nav_msgs::msg::Odometry>(
          "odom", 1, pub_options);

      // XL: create tf broadcaster (not sure if it is useful)
      tf_broadcaster_ =
          std::make_shared<tf2_ros::TransformBroadcaster>(this, 1, pub_options);

      // Create the subscribers
      color_subscriber_ = this->create_subscription<std_msgs::msg::ColorRGBA>(
          "qbot_led_strip", 1,
          std::bind(&QBotPlatformDriverInterface::color_callback, this, _1),
          sub_options);
      speed_cmd_subscriber_ =
          this->create_subscription<geometry_msgs::msg::Twist>(
              "cmd_vel", 1,
              std::bind(&QBotPlatformDriverInterface::speed_command_callback,
                        this, _1),
              sub_options);
      joint_cmd_subscriber_ =
          this->create_subscription<control_msgs::msg::JointJog>(
              "qbot_joint_speed_cmd", 1,
              std::bind(
                  &QBotPlatformDriverInterface::speed_joint_command_callback,
                  this, _1),
              sub_options);

      // RCLCPP_INFO(this->get_logger(), "driver_comm_sample_time.count: %ld",
      // driver_comm_sample_time.count());
      timer_ = this->create_wall_timer(
          driver_comm_sample_time,
          std::bind(&QBotPlatformDriverInterface::timer_callback, this),
          timer_cb_group_);
    } else {
 ...
 private:
  void timer_callback() {
    ...
    if (result >= 0) {
      ...
      int32_t sec = static_cast<int32_t>(receive_buffer[16]);  // seconds part
      uint32_t nsec = static_cast<uint32_t>((receive_buffer[16] - sec) *
                                            1e9);  // nanoseconds part
      rclcpp::Time receive_time(sec, nsec);

      // double t_stamp = receive_time.seconds();// + receive_time.nanoseconds()
      // * 1e-9; RCLCPP_INFO(this->get_logger(), "Time difference: = %f",
      // t_stamp-receive_buffer[16]); RCLCPP_INFO(this->get_logger(),
      // "\treceive_time = %f", t_stamp); RCLCPP_INFO(this->get_logger(),
      // "\treceive_buffer[16] = %f", receive_buffer[16]);

      if (!receive_recovered_sent) {
        RCLCPP_INFO(this->get_logger(),
                    "Data received from qbot_driver model.");
      ...
      // The above results in turning right giving positive angular speed, which
      // is incorrect!
      double vL = receive_buffer[2] * wheelRadius;
      double vR = receive_buffer[3] * wheelRadius;
      ...
      // Note: the angular velocity from the IMU is the negative of the
      // speed_feeback.twist.angular.z RCLCPP_INFO(this->get_logger(), "I %.4f",
      // receive_buffer[11]);
      ...
      // Pose (x, y, theta)
      double radiansLeft = receive_buffer[0] - prev_left_encoder;
      double radiansRight = receive_buffer[1] - prev_right_encoder;

      double distLeft = wheelRadius * radiansLeft;
      double distRight = wheelRadius * radiansRight;
      double avgDist = (distRight + distLeft) / 2;

      double dAngle = (distRight - distLeft) / wheelBase;
      double avgAngle = dAngle / 2 + z;
      ...
      x += cos(avgAngle) * avgDist;
      y += sin(avgAngle) * avgDist;
      z += dAngle;
      if (z > pi)
        z -= 2 * pi;
      else if (z <= -pi)
      ...
      odom.pose.pose.orientation.z = sin(z / 2);
      odom.pose.pose.orientation.w = cos(z / 2);

      // Build in uncertainty
      for (int i = 0; i < 36; i++) {
        if (i == 0 || i == 7)  // (x,y) position
          odom.pose.covariance[i] = .05;
        else if (i == 35)  // z heading
      ...
      // Velocity
      odom.twist.twist = speed_feeback.twist;

      // Update previous and publish
      prev_left_encoder = receive_buffer[0];
      prev_right_encoder = receive_buffer[1];
      odom_publisher_->publish(odom);

      if (pub_tf) {
        // RCLCPP_INFO(this->get_logger(), "odom_pred_dt: %f \n", odom_pred_dt);

        // XL: broadcast tf between base_link and odom
        tf2::Quaternion tf_quat;
        tf_quat.setRPY(0, 0, z + speed_feeback.twist.angular.z * odom_pred_dt);
        geometry_msgs::msg::Quaternion odom_quat = tf2::toMsg(tf_quat);

        auto odom_trans = geometry_msgs::msg::TransformStamped();
        odom_trans.header.stamp = receive_time;
        odom_trans.header.frame_id = "odom";
        odom_trans.child_frame_id = "base_link";
        odom_trans.transform.translation.x =
            x + cos(z) * speed_feeback.twist.linear.x * odom_pred_dt;
        odom_trans.transform.translation.y =
            y + sin(z) * speed_feeback.twist.linear.x * odom_pred_dt;
      ...
      if (!receive_warning_sent) {
        RCLCPP_INFO(this->get_logger(),
                    "Error (%d) receiving data from qbot_driver model. Make "
                    "sure the qbot_driver model is running.",
                    result);
    ...
    if (speed_command_counter_stop > 0) {
      // needed to reset the speed command in case we didn't get the
      // speed_command for a while.
    ...
    if (result < 0) {
      if (!send_warning_sent) {
        RCLCPP_INFO(this->get_logger(),
                    "Error (%d) sending data to qbot_driver model. Make sure "
                    "the qbot_driver model is running.",
                    result);
  ...
  void color_callback(const std_msgs::msg::ColorRGBA &led_color) {
    // RCLCPP_INFO(this->get_logger(), "Got RGBA value of: (%lf, %lf, %lf,
    // %lf)", led_color.r, led_color.g, led_color.b, led_color.a);
    send_buffer[1] =
        led_color.a;  // use the "a" as the boolean to use user LED color
  ...
  void speed_command_callback(const geometry_msgs::msg::Twist &twist) {
    ...
    // needed to reset the speed command in case we didn't get the speed_command
    // for a while.
    speed_command_counter = 0;

    // convert duration to number of times the communication to driver needs to
    // be set
    body_speed_duration =
        this->get_parameter("body_speed_duration").as_double();
    speed_command_counter_stop =
        (t_int)round(body_speed_duration * 1000.0 / 16.0);

    // RCLCPP_INFO(this->get_logger(), "In %s - got the message", __FUNCTION__);

    // RCLCPP_INFO(this->get_logger(), "In %s - speed_command_counter_stop: %d",
    // __FUNCTION__, speed_command_counter_stop);
  }

  void speed_joint_command_callback(
      const control_msgs::msg::JointJog &jointjog) {
    ...
    if (got_wheel_command) {
      ...
      // convert duration to number of times the communication to driver needs
      // to be set
      speed_command_counter_stop =
          (t_int)round(jointjog.duration * 1000.0 / 16.0);

      // RCLCPP_INFO(this->get_logger(), "In %s - speed_command_counter_stop:
      // %d", __FUNCTION__, speed_command_counter_stop);
    }
  }
  ...
  rcl_interfaces::msg::SetParametersResult set_parameters_callback(
    ...
    // Loop through the parameters....can happen if set_parameters_atomically()
    // is called
    for (const auto &parameter : parameters) {
      if (parameter.get_name().compare("driver_uri") == 0) {
        ...
        {
            result.successful = false;
            result.reason = "Cannot change this parameter while node is
        running.";
        }
        */
      } else if (parameter.get_name().compare("speed_limit_mode") == 0) {
        if ((parameter.as_int() < min_speed_limit_mode) ||
            (parameter.as_int() > max_speed_limit_mode)) {
          std::ostringstream error_stream;

          error_stream << "speed_limit_mode must be between "
                       << min_speed_limit_mode << " and "
                       << max_speed_limit_mode << ".";
        ...
        */
      } else if (parameter.get_name().compare("pub_tf") == 0) {
        if (node_running) {
          result.successful = false;
          result.reason = "Cannot change this parameter while node is running.";
        }
      } else if (parameter.get_name().compare("odom_pred_dt") == 0) {
        odom_pred_dt = parameter.as_double();
        // if (node_running)
        // {
        //     result.successful = false;
        //     result.reason = "Cannot change this parameter while node is
  ...
  rclcpp::Subscription<geometry_msgs::msg::Twist>::SharedPtr
      speed_cmd_subscriber_;
  rclcpp::Subscription<control_msgs::msg::JointJog>::SharedPtr
      joint_cmd_subscriber_;

  // --- XL: declare a tf broadcaster
  ...
  /* MC: Encoder info and attributes for odom */
  double x = 0.0;
  double y = 0.0;
  double z = 0.0;
  double prev_left_encoder = 0.0;
  double prev_right_encoder = 0.0;

  const t_double pi = 3.1415926535;
  /* --- */
  ...
  bool wheel_control = true;  // true: speed command to control wheels. false:
                              // speed command to control body
  bool arm_robot;
  bool hold;
  bool pub_tf;
  double odom_pred_dt;
...
int main(int argc, char *argv[]) {
  ...
  rclcpp::Node::SharedPtr qbp_driver_interface_node =
      std::make_shared<QBotPlatformDriverInterface>();

  // Get a multi-threaded executor
  rclcpp::executors::MultiThreadedExecutor executor;
  executor.add_node(qbp_driver_interface_node);

  RCLCPP_INFO(qbp_driver_interface_node->get_logger(),
              "Starting qbp_driver_interface loop...");
  executor.spin();
  RCLCPP_INFO(qbp_driver_interface_node->get_logger(),
              "qbp_driver_interface loop ended.\n");
